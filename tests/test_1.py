...
class MockResponseGET:
    ...
    def json(self):
        data = {"homeworks": [], "current_date": self.random_sid}
        return data
...
class TestHomework:
    ...
    def test_send_message(self, monkeypatch, random_sid):
        ...
        assert (
            len(signature(homework.send_message).parameters) == 1
    ...
    def test_get_homeworks(self, monkeypatch, random_sid, current_timestamp):
        ...
        assert hasattr(
            homework.get_homeworks, "__call__"
        ), "Функция `get_homeworks()` не существует. Не удаляйте её."
        assert (
            len(signature(homework.get_homeworks).parameters) == 1
        ), "Функция `get_homeworks()` должна быть с одним параметром."

        result = homework.get_homeworks(current_timestamp)
        assert type(result) == dict, (
            "Проверьте, что из функции get_homeworks() " "возвращается словарь"
        )
    ...
    def test_parse_homework_status(self, random_sid):
        ...
        assert hasattr(
            homework.parse_homework_status, "__call__"
        ), "Функция `parse_homework_status()` не существует. Не удаляйте её."
        assert (
            len(signature(homework.parse_homework_status).parameters) == 1
        ...
        assert result.endswith("Ревьюеру всё понравилось, работа зачтена!"), (
